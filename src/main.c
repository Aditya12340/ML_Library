...
// ALL THE FUCKING DECLARATIONS (cleaner way would be to make its own header file)
// but this is the only way I hit 1k lines (need to larp-maxx)


//data structure of a matrix: 
typedef struct{ // iniitalizing matrices
    u32 rows, cols; // these are the number of columns and rows
    f32* data; // one dimensional array for the matrix 
    // so indexing happens via data[i * cols + j]
} matrix;
...
// autodiff functions for creating nodes.
...
// functions declarations for ML math:
...
b32 mat_check2(const matrix* a, const matrix* b);
b32 mat_check3(const matrix* out, const matrix* a, const matrix* b);
...
matrix*mat_load(mem_arena* arena, u32 rows, u32 cols, const char* filename){
    ...
    sz = MIN(sz, sizeof(f32) * rows * cols);
    fread(mat->data, 1, sz, f);
    fclose(f);
...
bool mat_copy(matrix* dst, matrix* src){
    ...
    memcpy(dst->data, src->data, sizeof(*dst->data) * dst->rows * dst->cols);
    return true;
...
// filling a matrix with the values we want
void mat_fill(matrix* mat, f32 x){
    u64 sz = (u64) mat -> rows * mat -> cols;
    for(u64 i = 0; i < sz; i++){
        mat->data[i] = x;
    }
}
...
// scaling a matrix (so scalar multiplication)
void mat_scale(matrix*mat, f32 scale){
    u64 sz = (u64) mat -> rows * mat -> cols;
    for(u64 i = 0; i < sz; i++){
        mat->data[i] *= scale;
...
// diff variations of matrix multiplication//////////////////////
...
b32 mat_relu(matrix* out, const matrix* in){
    if (!mat_check2(out,in)) { return false; }
    u64 sz = (u64) in->rows * in->cols;
    for(u64 i = 0; i < sz; i++){
        out->data[i] = MAX(in->data[i],0);
    }
    return true;
} // 0 if <0, = to input if >0; helped fix the vanishing gradient problem.

b32 mat_softmax(matrix* out, const matrix* in){
    if (!mat_check2(out,in)) { return false; }
    u64 sz = (u64) in->rows * in->cols;
    // now magicalbat here implements a global softmax on the matrix without a mathematical stability shift 
    // i will implement the mathematical stability shift. this does increase the compile time a bit more but is still O(n)
    f32 max = in->data[0];
    for(u64 k = 1; k < sz; k++){
        if(in->data[k]> max){
            max = in->data[k];
...
b32 mat_cross_entropy(matrix* out, const matrix*p, const matrix* q){
    // this is a loss function. checking output vs input
    if (!mat_check3(out, p, q)) {
    return false;
    }
    u64 sz = (u64) q->rows * q->cols;
    for(u64 i = 0; i < sz; i++){
...
// function for matrix sz checker///////////////////////////////////
b32 mat_check2(const matrix* a, const matrix* b)
{
    return a != NULL &&
           b != NULL &&
...
void create_mnist_model(mem_arena* arena, model_context* model){
    ...
    model_var* W0 = mv_create(arena, model, 16, 784, parameter_flags);
    model_var* W1 = mv_create(arena, model, 16, 16,  parameter_flags);
    model_var* W2 = mv_create(arena, model, 10, 16,  parameter_flags);
    ...
    model_var* B0 = mv_create(arena, model, 16, 1, parameter_flags);
    model_var* B1 = mv_create(arena, model, 16, 1, parameter_flags);
    model_var* B2 = mv_create(arena, model, 10, 1, parameter_flags);
...
/* so now we embark on what is the more complicated part of this project. We are now building
an autodiff computation graph. */

model_var* mv_create(
    mem_arena* arena, model_context* model, u32 rows, u32 cols, u32 flags){
...
model_var* mv_relu(
    mem_arena* arena, model_context* model, model_var* input, u32 flags ){
        return _mv_unary_impl(arena, model, input, 
            input->val->rows, input->val->cols, flags, 
            MV_OP_RELU);
    }
...
model_var* mv_add(
    mem_arena* arena, model_context* model, model_var* a, model_var* b ,u32 flags ){
        if (!mat_check2(a->val, b->val)) {
            return false;
    }
        return _mv_binary_impl(arena, model, a, b,
            a->val->rows, a->val->cols, flags, 
            MV_OP_ADD);
    }
...
model_var* mv_cross_entropy(
    ...
    model_var* p, model_var* q ,u32 flags ){
         ...
         if ((p->val->cols != q->val->cols) || (p->val->rows != q->val->rows)){ return NULL;}
        return _mv_binary_impl(arena, model, p, q,
            p->val->rows, q->val->cols, flags, 
            MV_OP_CROSS_ENTROPY);
    }


// Model training functions
// implementing DFS to traverse through each node of the network
model_program model_prog_create( mem_arena* arena, model_context* model, model_var* out_var){
        ...
        while(stack_size > 0){
            // if the "++" is before, it returns AFTER the increment, and if its before
            // if before it returns BEFORE the increment.
...
void model_prog_compute(model_program* prog){
    for(u32 i = 0; i < prog->size; i++){
        ...
        switch(cur->op){
            ...
            case MV_OP_MATMUL:{mat_mul(cur->val, a->val, b -> val, 1, 0 ,0);} break;
            case MV_OP_CROSS_ENTROPY: {mat_cross_entropy(cur->val, a->val, b->val);}break;
        }
    }
}
...
void model_train(model_context* model,const model_training_desc* training_desc){
    ...
    u32 input_sz = train_images->cols;
    u32 num_tests = test_images->rows;
    u32 output_sz = train_labels->cols;
    u32 num_batches = num_examples / training_desc -> batch_size;

    mem_arena_temp scratch = arena_scratch_get(NULL, 0);

    u32* training_order = PUSH_ARRAY(scratch.arena, u32, num_examples);
    ...
    for(u32 epoch = 0; epoch < training_desc -> epochs; epoch++){
        ...
        for(u32 batch = 0; batch < num_batches; batch++){
            ...
            for(u32 i = 0; i < training_desc -> batch_size; i++){
                ...
                memcpy(model->input->val->data, 
                    train_images->data + index * input_sz, 
                    sizeof(f32) * input_sz);

                memcpy(model->desired_output->val->data, 
                    train_labels->data + index * output_sz, 
                    sizeof(f32) * output_sz);
        ...
        for(u32 i = 0; i < num_tests; i++){
            memcpy(
                model->input->val->data, test_images -> data + i * input_sz, 
                sizeof(f32) * input_sz
            );
            memcpy(
                model->desired_output->val->data, test_labels -> data + i * output_sz, 
                sizeof(f32) * output_sz
            );
